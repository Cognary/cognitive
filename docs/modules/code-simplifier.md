...
:material-auto-fix: 代码简化模块（v2 格式示例）

## 概述

`code-simplifier` 是一个使用 **v2 格式**的示例模块，展示了新格式的所有特性：
...
```bash
# 简化代码
cog run code-simplifier --args "x = 1 + 2 + 3"

# 指定语言
echo '{"code": "const x = a && a.b && a.b.c", "language": "javascript"}' | cog run code-simplifier

# 美化输出
cog run code-simplifier --args "for i in range(len(arr)): print(arr[i])" --pretty
```
  ...
  "changes": [
    {
      ...
      "scope": "function",  // local | function | file | project
