...
describe('QwenProvider request shaping (stable)', () => {
  ...
  it('adds response_format=json_object and appends schema guidance when jsonSchema is provided', async () => {
    const p = new QwenProvider('test-key', 'qwen3-max');
    ...
    expect(String(url)).toContain('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions');
