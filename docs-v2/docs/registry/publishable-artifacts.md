...
```bash
npx cogn@2.2.15 registry verify --remote --index https://github.com/<org>/<repo>/releases/latest/download/cognitive-registry.v2.json
```

For reproducible verification, pin a tag:

```bash
npx cogn@2.2.15 registry verify --remote --index https://github.com/<org>/<repo>/releases/download/vX.Y.Z/cognitive-registry.v2.json
```
