...
Cognitive Modules CLI is distributed via npm. The recommended entrypoint is `npx cogn@2.2.16 ...` (avoids PATH/binary conflicts on your machine).
...
npx cogn@2.2.16 run ./demo.md --args "hello" --profile core

# Force prompt-based structured output (useful when a provider rejects native schemas)
npx cogn@2.2.16 run ./demo.md --args "hello" --structured prompt

# Higher assurance
npx cogn@2.2.16 run code-reviewer --args "..." --profile standard

# Certified gate (refuses legacy modules + requires registry provenance/integrity)
npx cogn@2.2.16 run code-reviewer --args "..." --profile certified

# Force validation + write audit record
npx cogn@2.2.16 run code-reviewer --args "..." --profile standard --audit --validate on
```
...
```bash
npx cogn@2.2.16 run <module> --args "..." --structured prompt
```
...
```bash
npx cogn@2.2.16 init my-module
npx cogn@2.2.16 validate my-module --v22
```
...
npx cogn@2.2.16 registry build --tag vX.Y.Z

# Verify local tarballs match the v2 registry index
npx cogn@2.2.16 registry verify --index cognitive-registry.v2.json --assets-dir dist/registry-assets
...
npx cogn@2.2.16 registry verify --remote --index https://github.com/Cognary/cognitive/releases/download/vX.Y.Z/cognitive-registry.v2.json

# Tune remote verification limits (defaults: 15s, 2MB index, 25MB tarball)
npx cogn@2.2.16 registry verify --remote \
...
```bash
npx cogn@2.2.16 search "code" --registry-timeout-ms 15000 --registry-max-bytes 2097152
```
...
```bash
npx cogn@2.2.16 compose code-review-pipeline --args "code" --timeout 60000 --trace
npx cogn@2.2.16 compose-info code-review-pipeline
```
