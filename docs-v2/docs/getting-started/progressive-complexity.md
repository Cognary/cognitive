...
|---|---|---|---|
| **Repeatable outputs** (not just text) | `schema.json` (input/data/meta) | `npx cogn@2.2.16 validate --all` | Machine-checkable contracts; fewer prompt regressions |
| **Policy guardrails** (risk, enums, overflow) | `tier`, `schema_strictness`, `overflow`, `enums` | `npx cogn@2.2.16 run ...` (default validates) | Prevents silent drift; consistent behavior across CLI/HTTP/MCP |
| **Auditability** (post-incident analysis) | Require `meta.explain` + store envelopes/events | `npx cogn@2.2.16 run --stream` | NDJSON event stream; you can reconstruct end state + debug failures |
...
| **Distribution** (discover + install) | Registry index + Release tarballs | `npx cogn@2.2.16 search`, `npx cogn@2.2.16 add <registry>` | Deterministic installs and version pinning; safer sourcing model |
| **Interoperability claims** | Conformance tests + vectors | `npx cogn@2.2.16 test` | Evidence the runtime matches CEP behavior (not marketing) |
...
2. Team usage: commit modules + add `schema.json` + run `npx cogn@2.2.16 validate --all` in CI.
