...
|---|---|---|---|
| **Repeatable outputs** (not just text) | `schema.json` (input/data/meta) | `cog validate --all` | Machine-checkable contracts; fewer prompt regressions |
| **Policy guardrails** (risk, enums, overflow) | `tier`, `schema_strictness`, `overflow`, `enums` | `cog run ...` (default validates) | Prevents silent drift; consistent behavior across CLI/HTTP/MCP |
| **Auditability** (post-incident analysis) | Require `meta.explain` + store envelopes/events | `cog run --stream` | NDJSON event stream; you can reconstruct end state + debug failures |
...
| **Distribution** (discover + install) | Registry index + Release tarballs | `cog search`, `cog add <registry>` | Deterministic installs and version pinning; safer sourcing model |
| **Interoperability claims** | Conformance tests + vectors | `cog test` | Evidence the runtime matches CEP behavior (not marketing) |
...
2. Team usage: commit modules + add `schema.json` + run `cog validate --all` in CI.
