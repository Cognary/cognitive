...
This page uses the unambiguous entrypoint `npx cogn@2.2.13 ...` to avoid any PATH/binary conflicts on your machine.
...
`npx cogn@2.2.13 doctor` shows provider configuration status.
...
npx cogn@2.2.13 add Cognary/cognitive -m code-reviewer --tag v1.0.0
...
npx cogn@2.2.13 update code-simplifier --tag v2.0.0

# List available versions
npx cogn@2.2.13 versions Cognary/cognitive

# Remove module
npx cogn@2.2.13 remove code-simplifier
```
...
2. `~/.cognitive/modules/` (user global, installed by `npx cogn@2.2.13 add`)
