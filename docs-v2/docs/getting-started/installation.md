...
This page uses the unambiguous entrypoint `npx cogn@2.2.14 ...` to avoid any PATH/binary conflicts on your machine.
...
npx cogn@2.2.14 add Cognary/cognitive -m code-reviewer
...
```bash
npx cogn@2.2.14 core new demo.md
...
```bash
npx cogn@2.2.14 core promote demo.md
```
...
npx cogn@2.2.14 run ./demo-single-file.md --args "hello" --pretty
```
...
# Output: Cognitive Runtime v2.2.14

npx cogn@2.2.14 doctor
```

`npx cogn@2.2.14 doctor` shows provider configuration status.
...
npx cogn@2.2.14 add Cognary/cognitive -m code-reviewer --tag v1.0.0
...
npx cogn@2.2.14 update code-simplifier --tag v2.0.0

# List available versions
npx cogn@2.2.14 versions Cognary/cognitive

# Remove module
npx cogn@2.2.14 remove code-simplifier
```
...
2. `~/.cognitive/modules/` (user global, installed by `npx cogn@2.2.14 add`)
