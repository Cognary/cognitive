...
This page uses the unambiguous entrypoint `npx cogn@2.2.16 ...` to avoid any PATH/binary conflicts on your machine.
...
# Output: Cognitive Runtime v2.2.16

npx cogn@2.2.16 doctor
```

`npx cogn@2.2.16 doctor` shows provider configuration status.
...
2. `~/.cognitive/modules/` (user global, installed by `npx cogn@2.2.16 add`)
