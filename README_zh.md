...
- 推荐入口是 `npx cogn@2.2.14 ...`（避免你机器上存在其他 `cog` 命令导致误用）。
...
| `api-designer` | decision | REST API 设计 | `npx cogn@2.2.14 run api-designer --args "order system"` |
| `ui-spec-generator` | exploration | UI 规范生成 | `npx cogn@2.2.14 run ui-spec-generator --args "e-commerce homepage"` |
| `ui-component-generator` | exploration | UI 组件规范 | `npx cogn@2.2.14 run ui-component-generator --args "button component"` |
| `product-analyzer` | exploration | 产品分析 | `npx cogn@2.2.14 run product-analyzer --args "health product"` |
